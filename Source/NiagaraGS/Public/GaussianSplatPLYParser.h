...
private:
    ...
    struct FPLYHeader
    {
        ...
        // ── Precomputed byte offsets (filled once at end of ParseHeader) ──
        // OffsetOf() is a linear search; calling it per-splat for ~58 properties
        // across millions of splats is what made parsing take seconds. We resolve
        // every needed offset ONCE here and read by offset in the hot loop.
        int32 OffX = -1, OffY = -1, OffZ = -1;
        int32 OffScale[3] = { -1, -1, -1 };
        int32 OffRot[4]   = { -1, -1, -1, -1 };
        int32 OffOpacity  = -1;
        int32 OffDc[3]    = { -1, -1, -1 };
        TArray<int32> OffRest;   // one entry per f_rest_N, in order
