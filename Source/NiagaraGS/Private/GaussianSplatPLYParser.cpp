    ...
    FString& OutError)
{
    ...
    // ── Resolve every byte offset ONCE (avoids per-splat linear lookups) ──
    OutHeader.OffX = OutHeader.OffsetOf(TEXT("x"));
    OutHeader.OffY = OutHeader.OffsetOf(TEXT("y"));
    OutHeader.OffZ = OutHeader.OffsetOf(TEXT("z"));
    OutHeader.OffScale[0] = OutHeader.OffsetOf(TEXT("scale_0"));
    OutHeader.OffScale[1] = OutHeader.OffsetOf(TEXT("scale_1"));
    OutHeader.OffScale[2] = OutHeader.OffsetOf(TEXT("scale_2"));
    OutHeader.OffRot[0] = OutHeader.OffsetOf(TEXT("rot_0"));
    OutHeader.OffRot[1] = OutHeader.OffsetOf(TEXT("rot_1"));
    OutHeader.OffRot[2] = OutHeader.OffsetOf(TEXT("rot_2"));
    OutHeader.OffRot[3] = OutHeader.OffsetOf(TEXT("rot_3"));
    OutHeader.OffOpacity = OutHeader.OffsetOf(TEXT("opacity"));
    OutHeader.OffDc[0] = OutHeader.OffsetOf(TEXT("f_dc_0"));
    OutHeader.OffDc[1] = OutHeader.OffsetOf(TEXT("f_dc_1"));
    OutHeader.OffDc[2] = OutHeader.OffsetOf(TEXT("f_dc_2"));

    OutHeader.OffRest.SetNumUninitialized(OutHeader.SHRestCount);
    for (int32 i = 0; i < OutHeader.SHRestCount; ++i)
    {
        OutHeader.OffRest[i] = OutHeader.OffsetOf(FString::Printf(TEXT("f_rest_%d"), i));
    }
    ...
    const FPLYHeader& Header)
{
    ...
    // Read by precomputed offset — no string lookups in this hot path.
    auto AtOff = [&](int32 Off, float Default = 0.0f) -> float
        {
            return (Off >= 0) ? ReadFloat(ElementData, Off) : Default;
        };

    // Position
    Out.Position = ConvertPosition(
        AtOff(Header.OffX), AtOff(Header.OffY), AtOff(Header.OffZ));

    // Scale (log-space in PLY)
    Out.Scale = ConvertScale(
        AtOff(Header.OffScale[0]), AtOff(Header.OffScale[1]), AtOff(Header.OffScale[2]));

    // Orientation (PLY: rot_0=w, rot_1=x, rot_2=y, rot_3=z)
    Out.Orientation = ConvertOrientation(
        AtOff(Header.OffRot[0], 1.0f),
        AtOff(Header.OffRot[1]), AtOff(Header.OffRot[2]), AtOff(Header.OffRot[3]));

    // Opacity (logit → sigmoid)
    Out.Opacity = Sigmoid(AtOff(Header.OffOpacity));

    // Base color from DC SH coefficients
    Out.Color = SHDCToColor(
        AtOff(Header.OffDc[0]), AtOff(Header.OffDc[1]), AtOff(Header.OffDc[2]));

    // ── Higher-order SH coefficients (raw, channel-major as stored in PLY) ──
    // Layout for degree 3 (45 floats): R_0..14, G_0..14, B_0..14. Re-interleaving
    // to GPU basis-major happens in UploadDataForInstance.
    {
        const int32 CoeffsToCopy = Header.OffRest.Num();
        Out.SHCoefficients.SetNumUninitialized(CoeffsToCopy);
        for (int32 i = 0; i < CoeffsToCopy; ++i)
        {
            Out.SHCoefficients[i] = AtOff(Header.OffRest[i], 0.0f);
        }
    }
